...
function renderizarLista() {
    ...
    if (filtroTamano !== "todos") {
        huecosFiltrados = huecosFiltrados.filter((h) => h.tamano === filtroTamano);
    }
    if (busqueda) {
    ...
    contenedor.innerHTML = huecosFiltrados
        ...
        <div class="tarjeta-reporte" data-id="${h.id}">
            ...
            <div style="display:flex; flex-direction:column; gap:0.3rem; align-items:flex-end">
                ...
                ${h.tamano ? `<span class="tamano-badge">${ETIQUETA_TAMANO[h.tamano] || h.tamano}</span>` : ""}
            </div>
        </div>`
        )
        .join("");
...
document.getElementById("filtroTamano").addEventListener("change", renderizarLista);
...
document.getElementById("btnExportar").addEventListener("click", () => {
    const datos = listaHuecosCompleta.map((h) => ({
        ...
        Tamano: ETIQUETA_TAMANO[h.tamano] || h.tamano || "",
        Material: ETIQUETA_MATERIAL[h.material] || "",
