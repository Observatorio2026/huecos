...
create table if not exists public.huecos (
    ...
    material          text
                        check (material is null or material in ('asfalto', 'concreto_rigido', 'adoquin', 'destapado', 'otro')),
...
-- Si la tabla ya existía de una versión anterior del proyecto (sin tamaño/material),
-- ejecuta estas dos líneas para agregarlas sin perder los datos existentes:
-- alter table public.huecos add column if not exists tamano text not null default 'mediano' check (tamano in ('pequeno','mediano','grande'));
-- alter table public.huecos add column if not exists material text check (material is null or material in ('asfalto','concreto_rigido','adoquin','destapado','otro'));
...
-- =====================================================================
-- Nota: CREATE OR REPLACE VIEW solo permite agregar columnas al final, no
-- insertarlas en medio ni renombrarlas. Por eso se elimina primero la vista.
drop view if exists public.huecos_geojson;

create view public.huecos_geojson as
select
    id,
    estado,
    direccion,
...
-- Nota: no se puede usar CREATE OR REPLACE si cambia el tipo de fila de salida
-- (columnas devueltas). Por eso se elimina primero la función si ya existe.
drop function if exists public.huecos_con_coordenadas();

create function public.huecos_con_coordenadas()
returns table (
    id uuid,
    estado text,
    direccion text,
...
as $$
    select
        id, estado, direccion, descripcion, tamano, material, comentario, foto_url, notas_admin,
